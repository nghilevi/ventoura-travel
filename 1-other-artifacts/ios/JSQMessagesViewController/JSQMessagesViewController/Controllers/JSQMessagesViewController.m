...
- (void)jsq_configureMessagesViewController
{
    ...
    self.emoticonViewVisible = NO;
    self.keyboardViewVisible = NO;
...
- (void)keyboardController:(JSQMessagesKeyboardController *)keyboardController keyboardDidChangeFrame:(CGRect)keyboardFrame
{
    ...
    NSLog(@"changed %f",heightFromBottom);

    heightFromBottom = MAX(0.0f, heightFromBottom);
    if(!self.emoticonViewVisible){
        [self jsq_setToolbarBottomLayoutGuideConstant:heightFromBottom];
    }
}

- (void)didPressEmoticion:(BOOL)showEmoticons
{
    CGFloat heightFromBottom = 0.0;
    if (showEmoticons) {
        heightFromBottom = 200.0f;
    }
    NSLog(@"pressed %f",heightFromBottom);
...
- (void)jsq_setToolbarBottomLayoutGuideConstant:(CGFloat)constant
{
    ...
    [self.view layoutIfNeeded];
    NSLog(@"jsq_setToolbarBottomLayoutGuideConstant");
