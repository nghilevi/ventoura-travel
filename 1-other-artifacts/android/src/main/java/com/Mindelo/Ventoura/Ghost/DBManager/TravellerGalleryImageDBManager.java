...
public class TravellerGalleryImageDBManager extends DAO<ImageProfile> {

	private SQLiteDatabase db;

	public TravellerGalleryImageDBManager(SQLiteDatabase db) {
		super(db);
		this.db = db;
	}

	public void setImageAsPortal(long travellerId, long imageId) {

		ContentValues args1 = new ContentValues();
		args1.put(DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY_FIELD_IS_PORTAL,
				false);
		String condition1 = DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY_FIELD_TRAVELLER_ID
				+ "=" + travellerId;
		this.db.update(DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY, args1,
				condition1, null);

		ContentValues args2 = new ContentValues();
		args2.put(DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY_FIELD_IS_PORTAL,
				true);
		String condition2 = DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY_FIELD_ID
				+ "=" + imageId;
		this.db.update(DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY, args2,
				condition2, null);
	}
	...
	public void save(ImageProfile entity) {
		db.execSQL(
				"INSERT INTO " + DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY
						+ " VALUES(?, ?, ?, ?)",
	...
	public ImageProfile loadSingleEntityFromCursor(Cursor cursor) {
		...
		imageProfile
				.setId(cursor.getLong(cursor
						.getColumnIndex(DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY_FIELD_ID)));
		imageProfile
				.setUserId(cursor.getLong(cursor
						.getColumnIndex(DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY_FIELD_TRAVELLER_ID)));
		if (cursor
				.getInt(cursor
						.getColumnIndex(DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY_FIELD_IS_PORTAL)) == 0) {
		...
		imageProfile
				.setImageContent(cursor.getBlob(cursor
						.getColumnIndex(DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY_FIELD_CONTENT)));
	...
	public void deleteMany(String condition) {
		delete(condition, DBConstant.TABLE_TRAVELLER_PROFILE_GALLERY);
	}
